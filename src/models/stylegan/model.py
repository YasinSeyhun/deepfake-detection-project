...
class StyleGANGenerator(nn.Module):
    def __init__(
        ...
        self.conv1 = StyledConv(
            self.channels[4], self.channels[4], 3, style_dim=latent_dim, blur_kernel=blur_kernel
        )
        self.to_rgb1 = ToRGB(self.channels[4], latent_dim, upsample=False)
        
        self.log_size = int(math.log2(1024))
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(
                    out_channel, out_channel, 3, style_dim=latent_dim, blur_kernel=blur_kernel
                )
            )
    ...
    def mean_latent(self, n_latent):
        latent_in = torch.randn(
            n_latent, self.latent_dim, device=self.input.input.device
        )
